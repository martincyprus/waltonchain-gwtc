...
// PublicWtcAPI provides an API to access Wtc related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicWtcAPI struct {
	b Backend
}

// NewPublicWtcAPI creates a new Wtc protocol API.
func NewPublicWtcAPI(b Backend) *PublicWtcAPI {
	return &PublicWtcAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicWtcAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Wtc protocol version this node supports
func (s *PublicWtcAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicWtcAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Wtc Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Wtc Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Wtc ECDSA signature for:
// keccack256("\x19Wtc Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/wtc/go-wtc/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Wtc Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/wtc/go-wtc/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Wtc signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Wtc blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
//
// https://github.com/wtc/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Wtc APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Wtc APIs exposed over the private
// debugging endpoint.
...
// Version returns the current wtc protocol version.
