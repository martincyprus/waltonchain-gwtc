...
type LightChain interface {
	...
	InsertHeaderChain(types.Blocks) (int, error)
...
func New(mode SyncMode, stateDb wtcdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() wtc.SyncProgress {
...
// 					lightblocks := make([]*types.Block, len(chunk))	
// 					for i,h := range chunk {
// 						lightblocks[i] = d.blockchain.GetBlockByHash(h.Hash())
// 						fmt.Println(i,lightblocks[i])
// 					}
// //-------------------------------------------------------------------------------------------------------------------------
// 					// if n, err := d.blockchain.InsertHeaderChain(chunk); err != nil {
// 					if n, err := d.lightchain.InsertHeaderChain(lightblocks); err != nil {
