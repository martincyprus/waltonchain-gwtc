...
#
# The cluster contains a bootnode, a gwtc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gwtc and
# swarm nodes
start_bootnode() {
...
# start_gwtc_node starts a gwtc node with --datadir pointing at <base-dir>/gwtc
# and a single, unlocked account with password "gwtc"
start_gwtc_node() {
  local dir="${base_dir}/gwtc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gwtc account address"
  fi
  ...
  start_node "gwtc" "${GETH_IP}" "$(which gwtc)" ${args[@]}
}
...
create_account() {
  ...
  gwtc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
