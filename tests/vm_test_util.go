...
// See https://github.com/wtc/tests/wiki/VM-Tests for the test format specification.
type VMTest struct {
	json vmJSON
}
...
func (t *VMTest) newEVM(statedb *state.StateDB, vmconfig vm.Config) *vm.EVM {
	...
	transfer := func(db vm.StateDB, sender, recipient common.Address, amount,Num,Time *big.Int) {}
	context := vm.Context{
