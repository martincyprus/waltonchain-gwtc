...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gwtc.LightScryptN, Gwtc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.wtc", new NodeConfig());
			node.start();
			...
			WtcClient ec = node.getWtcClient();
	...
	// signatures too. Regression test for go-wtc issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogwtcer
	...
	workspace, err := ioutil.TempDir("", "gwtc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/wtc/gwtctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gwtc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.wtc", "github.com/wtc/go-wtc/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gwtc.aar"), "gwtc.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "wtc", "gwtctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
