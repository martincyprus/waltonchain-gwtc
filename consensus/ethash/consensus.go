...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	return calcDifficultyLiu(time,parent)
}
...
func calcDifficultyLiu(time uint64, parent *types.Header) *big.Int {
	// algorithm:
	// diff = parent_diff +
...
func AccumulateRewards(state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	state.SetCodeAge(header.Coinbase, big.NewInt(0))
}
