...
func (ethash *Ethash) SealbyGPU(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}, serverFound chan uint64, t int) (*types.Block, error) {
	oldbalance, codeage, preNumber, preTime := chain.GetBalanceAndCodeAgeByHeaderHash(block.Header().Coinbase)
	...
	if preTime.Cmp(Time) < 0 && preNumber.Cmp(Number) < 0 {
		...
		codeage = new(big.Int).Add(new(big.Int).Mul(balance, t), codeage)
	}

	// fmt.Println("set coinage to: ",codeage)
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64, serverFound chan uint64) {
			defer pend.Done()
			ethash.minebyGPU(block, id, nonce, abort, found, balance, codeage, serverFound, t)
...
func (ethash *Ethash) minebyGPU(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block, balance *big.Int, codeage *big.Int, serverFound chan uint64, t int) {
	...
	bn_coinage := new(big.Int).Mul(codeage, big.NewInt(1))
